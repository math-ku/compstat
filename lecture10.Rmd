...
impossible to compute analytically and difficult and expensive to compute numerically.
...
Marginal likelihood is (weakly) increased in each iteration.
...
Thus
$$Q(p \mid p') = E_{p'}( \ell_{\textrm{complete}}(p) \mid X = x) = \sum_{i=1}^K E_{p'}( Y_i \mid X = x) \log(p_i)$$
for any $X = M(Y)$.
...
$$E_p (Y_k \mid X = x) = \frac{x_j p_k}{M(p)_j}.$$
for $k \in A_j$.

--

The abstract E-step of the EM algorithm can be implemented as
...
Which is $\hat{p} = \frac{1}{n} \mathbf{X} y$ for us.
...
The MLE of the complete log-likelihood is a linear estimator in $y / n$,
and we implement an abstract M-step as any such linear estimator.
...
`EStep_mult()` and `MStep_mult()` are abstract implementations. Require
...
see [Algorithms and Convergence](https://cswr.nrhstat.org/7-1-algorithms-and-convergence.html)).
...
For statistics we need the observed Fisher information (Hessian of the negative log-likelihood for the
observed data).
...
Here $\hat{i}_Y$ is directly computable from $Q$ while it is not clear how to
compute $\hat{i}_{Y \mid X}$.
...
We use the numDeriv functions `jacobian()` and `hessian()` to differentiate numerically.

Either numerically differentiate the function twice:
...
```{r tracing-convergence, results = "hide"}
library(CSwR)
