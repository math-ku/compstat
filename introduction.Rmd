...
```{r logcial_vector_mean, results = "hold"}
...
bench::mark(
  {
    y <- numeric(length(x))
    for (i in seq_along(x)) y[i] <- 10 * x[i]
    y
  },
  y <- 10 * x
)
...
library(tidyverse)
mutate(kern_benchmarks, fun = as.character(expression)) |>
  ggplot(aes(m, median, color = fun)) +
  geom_point() +
  geom_line() +
