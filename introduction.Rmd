...
There are good tools for debugging in R, especially in R Studio.
...
Our `count_zeros()` function is correct but not very efficient.

```{r}
system.time(count_zeros(rpois(1e7, 2)))
```
...
* If not, then profile, benchmark, and debug. Then refactor and optimize.
--
