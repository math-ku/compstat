...
$$x_{n+1} = x_n - \gamma \nabla f_{i_n}(x_n)$$
where $i_n$ is an index in $\{1, \ldots, n\}$ drawn at random.
]
...
Idea: take (mini-)batch of size $b$ of the data and iterate through
$$x_{k+1} = x_k - \frac{\gamma}{b} \sum_{i=1}^b \nabla f_i(x_k).$$
...
$$\gamma_k = \frac{\gamma_0 K}{K + n^a} = \frac{\gamma_0 }{1 + K^{-1} k^{a}}$$
...
the $Z_i$s uniformly distributed in $(-1, 1)$,
...
- Implement a stochastic gradient descent algorithm to fit standard least-squares
...
- Use the **mvtnorm** package to generate $X$ from a multivariate Gaussian distribution,
  specifying some correlation between the variables.
--

- Try to visualize both the convergence of the algorithm and the path it takes in the parameter space.
...
Test by comparing to standard batch solution:
...
- Use Rcpp to speed up the optimization process
