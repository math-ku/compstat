...
Let's profile the following reimplementation of `gauss()`.
...
- Run `profvis()` on the expression you want to profile. (Or use the RStudio tool.)
- The result is an interactive webpage (opens in tab in RStudio).

```{r source_profvis, cache = FALSE, eval = FALSE}
source("R/gauss.R", keep.source = TRUE)
x <- rnorm(1e7)
...
```{r source_profvis_result, ref.label="source_profvis", cache = FALSE, eval = TRUE, echo = FALSE}
```
...
```{r profile-kern-dens-detail-output, ref.label = "profile_kern_dens_detail", echo = FALSE}
```
...
Typical approach: find bottleneck through profiling and then (micro)benchmark alternatives.
...
If $n < m$, then binning will not help
...
```{r matvec-mul-ex-vectorize, include = FALSE}
...
**Recall:** The OLS estimator is given by $\hat{\beta} = (X'X)^{-1}X'y$.
...
Benchmark the future-based version against the standard version.

```{r future-benchmark, include = FALSE, cache = TRUE}
# n <- 100
# p <- 10
# x <- matrix(rnorm(n * p), n, p)
# y <- rnorm(n)
#
# ols_future <- function(x, y) {
#   xtx %<-% crossprod(x)
#   xty <- crossprod(x, y)
#
#   solve(xtx, xty)
...
```{r future-benchmark-result, include = FALSE, cache = TRUE}
# bench::mark(
#   ols_future(x, y),
#   ols_standard(x, y)
# )
