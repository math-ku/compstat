...
This assignment uses the Poisson data available [here](/data/poisson.csv). It is
a table with 100 rows and two variables, $z$ and $x$, where $z$ takes integer
values and $x$ takes positive real values.

The purpose of this assignment is to sample from the probability distribution on
$[0, \infty)$ with density
$$
  f(y) \propto \prod_{i=1}^{100} \exp\left( yz_ix_i - e^{yx_i} \right),
  \quad y \geq 0.
$$
...
*For those interested, the distribution given by $f$ above is a simple example
of a Bayesian posterior distribution in a Poisson regression model.*
...
Think of $X_k$ as the capital that a company or a gambler earns or loses at time
$n$. Starting with a total capital of 30, $S_n$ is the total capital at time
$n$. We are interested in computing the *ruin probabilities* before time $n$:
$$
  p(n) = \Pr(\exists k \leq n: S_k \leq 0).
$$
...
$$
  g_{\theta, n}(x)
  = \frac{1}{\varphi(\theta)^n} \exp\left( \theta \sum_{k=1}^n x_k \right)
$$
...
Implement importance sampling to compute $p(100)$ by sampling from
$g_{\theta, 100}$ for suitable $\theta$. Is there an optimal choice of $\theta$?

::: {.callout-note}
You should consider number of samples as well as run time to discuss what
"optimal" means.
:::
