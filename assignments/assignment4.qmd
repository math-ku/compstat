...
# Plot: points and fitted logistic curve
ggplot(df, aes(x = x, y = y)) +
  ...
  theme_void()
ggsave(here::here("assets", "images", "assignment4.svg"))
```
...
the different algorithms such as size of mini-batch and learning rate for SGD.

To better understand what happens inside the solvers, initially fix all but two
of the model parameters (basis coefficients in Assignment A) and investigate the
resulting two-dimensional optimization problem. Visualize the objective function
and the parameter trajectories of the algorithms. Consider what these plots
reveal about the effects of starting values and algorithmic control parameters,
and use those insights when working with the full problem.

You do not need to investigate every possible combination of parameters. It is
fine to focus on a particular aspect of the algorithms, such as momentum,
learning rate, or mini-batch size.
...
$$
  \log \frac{p_i(\beta)}{1 - p_i(\beta)}
  = f(x_i \mid \beta) = (\varphi_1(x_i), \ldots, \varphi_p(x_i))^T \beta
$$
...
$$
  H(\beta)
  = -\frac{1}{N} \sum_{i=1}^N \Big(y_i \log p_i(\beta) + (1 - y_i) \log (1 - p_i(\beta)) \Big)
             + \lambda \| f_{\beta}'' \|_2^2
$$
...
the polynomial basis), affect convergence.

Visualize the data and fitted curves. For simulated data, also show the
data-generating curve, and investigate how the choice of starting values affects
...
$$
  ...
  = \gamma + \frac{\delta - \gamma}{1 + e^{\beta \log(x)-\alpha}}.
$$
Here, $\alpha, \beta, \gamma, \delta \in \mathbb{R}$.

In this assignment you will consider the model with observations
$$
  Y_i = f(x_{i} \mid \alpha, \beta, \gamma, \delta) + \varepsilon_{i}
$$
...
nonlinear least squares, that is, by minimizing
$$
  \frac{1}{N} \sum_{i=1}^N \big(y_i - f(x_i \mid \alpha, \beta, \gamma, \delta)\big)^2.
$$
