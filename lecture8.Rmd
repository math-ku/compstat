...
Typically called global optimization methods; assume very little.
...
Algorithm will converge very slowly.

#### Too large

Algorithm might diverge (or oscillate).
...
```{r newton-vs-gd-poisson, echo = FALSE}
d1 <- trace_sum_gd |>
  select(value, time = .time) |>
  mutate(iteration = seq_along(time), method = "Gradient Descent")


d2 <- trace_sum_newton |>
  select(value, time = .time) |>
  mutate(iteration = seq_along(time), method = "Newton")

trace_sum_merged <- bind_rows(d1, d2)

ggplot(trace_sum_merged, aes(time, value - val_min + 1e-8, color = method)) +
  geom_line() +
  scale_y_log10() +
  labs(color = NULL, y = "Suboptimality")
```
