...
With $S_{ij} = \frac{1}{k} 1_{N_i}(j)$ and $\mathbf{S} = (S_{ij})$ 
...
The R function `filter()` applies linear filtering (moving averages or autoregression).
...
Thus
$$\operatorname{MSE}(x) = \underbrace{\left[f(x) - \frac{1}{k} \sum_{l \in N(x)} f(x_l)\right]^2}_{\text{squared bias}} 
...
```{r loess, fig.height=5, fig.width = 10}
p + geom_smooth()
```
