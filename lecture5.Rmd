...
The R default is the 32-bit *Mersenne Twister*, which generates integers in the range
$$\{0, 1, \ldots, 2^{32} -1\}.$$
...
There are many algorithms for generating Gaussian random variables.

* [Box-Muller](https://en.wikipedia.org/wiki/Box%E2%80%93Muller_transform): A 
transformation of two independent uniforms into two independent Gaussian random
variables (polar coordinates).
--

* [Inversion](https://en.wikipedia.org/wiki/Inverse_transform_sampling). $X = \Phi^{-1}(U)$
where $\Phi$ is the distribution function for the Gaussian distribution.
--


* [Rejection sampling](https://cswr.nrhstat.org/4-3-reject-samp.html). 
See e.g. [Exercise 4.1 in CSwR](https://cswr.nrhstat.org/4-5-univariate-ex.html) or the 
[Ziggurat algorithm](https://en.wikipedia.org/wiki/Ziggurat_algorithm).

--

```{r rng-kind, eval=TRUE}
RNGkind()
```
...
Write a test for your implementation.
...
```{r vMsim2, fig.pos="h", fig.show='hold', out.width="50%"}
...
r <- 10
bench::mark(gammasim(1000, r), gammasim_vec(1000, r), rgamma(1000, r), check = FALSE) %>% autoplot()
```
...
hist(Betasim_vec(100000, x1 = 0.3, x2 = 0.7, alpha = 4, beta = 2),
...
hist(Betasim_vec(100000, x1 = 0.3, x2 = 0.7, alpha = 1.8, beta = 2.4),
  prob = TRUE, main = "", ylim = c(0, 3)
)
...
hist(Betasim_vec(100000, x1 = 0.2, x2 = 0.5, alpha = 4, beta = 2),
  prob = TRUE, main = "", ylim = c(0, 3)
)
curve(x^3 * (1 - x) / beta(4, 2), col = "blue", lwd = 2, add = TRUE)
...
hist(Betasim_vec(100000, x1 = 0.2, x2 = 0.5, alpha = 1.8, beta = 2.4),
  prob = TRUE, main = "", ylim = c(0, 3)
)
curve(x^0.8 * (1 - x)^1.4 / beta(1.8, 2.4), col = "blue", lwd = 2, add = TRUE)
