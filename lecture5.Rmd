...
**Classical example:** If $F^{-1} : (0,1) \mapsto \mathbb{R}$ is the
...
We want to sample from a target distribution $f(x)$ but we can only (or want to) sample from a proposal $g(x)$.
...
$$Z_0 = R \cos(\Theta) = \sqrt{-2 \log U_1} \sin (2\pi U_2)$$
and
