...
A gradient descent implementation and other code for today are in a
[source file](R/debugging.R).
...
- Downloading and open [`debugging.R`](R/debugging.R) in RStudio.
- Create another script with the code from these slides simulating $X$ and $y$ and
with the implementation of `objective()`, its gradient, and Hessian.
- Source `debugging.R` and test that `gradient_descent()` works. Now try `newton_method()` and observe the error.
...
- Source the fixed code and try evaluating the `newton_method()` call again. What happens?
- Insert another breakpoint at a suitable place.
- Explain and fix the bug. Remove the breakpoint.
- Test that the `newton_method()` implementation now works.
- Implement a `verbosity` option for the `newton_method()` that
  prints values from within the function during iterations.
...
.pull-right[
```{r, echo = FALSE, fig.width = 5, fig.height = 5.2, warning = FALSE}
other_negloglik <- function(theta, x) {
  theta <- c(theta, 0)

  # -crossprod(theta, x) + sum(x) * log(1 + sum(exp(theta)))

  pC <- exp(theta[1]) / sum(exp(theta))
  pI <- exp(theta[2]) / sum(exp(theta))
  pT <- 1 / sum(exp(theta))

  # TODO: This can improved by excplitly working out math
...
# x <- c(85, 196, 341)
# res <- optim(c(1 / 3, 1 / 3), other_negloglik, x = x)
# theta <- c(res$par, 0)
# phi <- sum(exp(theta))
# exp(theta) / phi
n <- 100
...
constrOptim(c(0.3, 0.3), neg_loglik_pep, NULL, ui = A, ci = b, x = x)$par
```

--

#### Still Requires Feasible Initial Points
