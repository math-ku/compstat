...
**Question**: With the basics of C++ in hand, it's now a great time to practice
...
**A**: We can try running each function and seeing what they do. For example, we
...
C++. For now, assume the inputs have no missing values. + `all()`. +
`cumprod()`, `cummin()`, `cummax()`. + `diff()`. Start by assuming lag 1, and
then generalise for lag n. + `range()`. + `var()`. Read about the approaches you
can take on Wikipedia. Whenever implementing a numerical algorithm, it's always
good to check what is already known about the problem.
...
**Question**: Rewrite any of the functions from the first exercise of Section
25.2.6 to deal with missing values. If na.rm is true, ignore the missing values.
If na.rm is false, return a missing value if the input contains any missing
values. Some good functions to practice with are `min()`, `max()`, `range()`,
`mean()`, and `var()`.
...
**Question**: Rewrite `cumsum()` and `diff()` so they can handle missing values.
...
**Question**: How can we use **currying (function composition and partial
application)** to create flexible and reusable kernel density estimation
functions?

**A**: In U2.qmd, we implemented kernel density estimation with the Epanechnikov
kernel. Now let's see how currying can make our code more modular and elegant.
...
Now comes the magic of currying! We can create specialized functions by
"currying" our general function:

```{r}
# Curry by kernel type - creates functions that use specific kernels
epanechnikov_kde <- function(x, h, n = 512) {
  kern_dens_general(x, h, epanechnikov_kernel, n)
}

gaussian_kde <- function(x, h, n = 512) {
  kern_dens_general(x, h, gaussian_kernel, n)
}

uniform_kde <- function(x, h, n = 512) {
  kern_dens_general(x, h, uniform_kernel, n)
}
...
df_bandwidths <- data.frame(
  ...
  bandwidth = rep(
    c("Narrow (0.3)", "Medium (0.8)", "Wide (1.5)"),
    each = length(dens_narrow$x)
  )
)
...
df_kernels <- data.frame(
  ...
  kernel = rep(
    c("Epanechnikov", "Gaussian", "Uniform"),
    each = length(dens_epa$x)
  )
)
...
p1 <- ggplot(df_bandwidths, aes(x = x, y = y, color = bandwidth)) +
  geom_line(size = 1) +
  labs(
    title = "Effect of Bandwidth (Epanechnikov Kernel)",
    x = "log(F12)",
    y = "Density",
    color = "Bandwidth"
  ) +
...
1. **Modularity**: Each component (kernel, bandwidth) can be modified
   independently
2. **Reusability**: Once created, curried functions can be reused multiple times
3. **Readability**: Code becomes more self-documenting (e.g., `epa_narrow`
   clearly indicates both kernel and bandwidth)
4. **Flexibility**: Easy to create new combinations without rewriting core logic
5. **Functional Programming**: Demonstrates higher-order functions and partial
   application
...
bandwidth_comparison <- function(data, bandwidths) {
  kde_functions <- lapply(
    bandwidths,
    function(bw) make_bandwidth_kde(epa_kde, bw)
  )
...
This example shows how currying transforms our kernel density estimation from a
monolithic function into a flexible, composable system. It's the same principle
we used with the `.na.rm_decorator` - we're creating specialized functions from
more general ones!
