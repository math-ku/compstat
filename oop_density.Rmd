...
How to create non-parametric density estimates using density kernel estimation
...
.pull-left[
We add a Gaussian density kernel for each point.
]
...
Take the `kern_dens()` function we started out with and implement Silverman's rule of thumb for choosing the bandwidth. Test against the default behavior of `density()` to check that your implementation is working.
