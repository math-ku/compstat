...
If $Y$ has density $g$ and $U$ is uniform on $(0,1)$, independently of $Y$, then
the conditional distribution
$$
  Y \mid U \leq \alpha' \frac{q(Y)}{p(Y)}
$$
has density $f$.
...
\begin{algorithm}[H]
  \caption{Rejection sampling to simulate a random variable $X$ with density
...
$\hat{\sigma}^2_{\mathrm{MC}} / N$ and an approximate 95% confidence interval for
$\mu$ is
$$
...
moment-generating functions. Plugging in a variance estimate loses the guarantee.
...
#|   distribution using a Student-t proposal with 3 degrees of freedom,
#|   location 10, and scale 3.
#| fig-width: 5
#| fig-height: 2.5
...
## Gamma Normalized Weights, Variance
...
#| fig-width: 4.5
#| fig-cap: >-
#|   Importance sampling estimate of the mean of a Gamma(8,1) distribution with
#|   approximate pointwise 95% confidence intervals using normalized weights.
...
More samples may help with poor overlap. They cannot repair missing support:
if $g = 0$ where $hf \neq 0$, ordinary IS misses part of the integral.

\medskip\pause

Redesign the importance density to cover the required support and put more
mass where the integrand is large. Self-normalized IS must cover the target
support, including where $h = 0$.
...
$$
  w^*(x) = \frac{f(x)}{g(x)}
$$
but ordinary IS variance depends on $h(X)w^*(X)$, not just the weights.
For self-normalized IS, the asymptotic variance depends on
$(h(X) - \mu)w^*(X)$.
...
The connection comes from the second-order Taylor expansion of $\log f(x)$ at
an interior mode $x^*$,
$$
  \log f(x) \approx \log f(x^*) - \frac{1}{2}(x - x^*)^T \Lambda (x - x^*)
$$
where $\Lambda$ is the Hessian of $-\log f(x)$ at $x^*$, assumed positive definite.
...
3. If $\Lambda$ is positive definite, use $\mathcal{N}(x^*, \Lambda^{-1})$ as the
   importance distribution.
...
3. Fit $g_{n+1}$ using the weighted samples, then increment $n$.
   \pause
...
plot_step <- function(iter) {
  ...
  ggplot(
    ...
    labs(
      y = "Density",
      subtitle = sprintf("ESS = %.0f, CV = %.1f", ess, cv)
    )
}
```

```{r}
#| label: adaptive-importance-fig1
#| echo: false
#| fig-width: 5.4
#| fig-height: 2.5
...
```{r}
#| label: adaptive-importance-fig2
#| echo: false
#| fig-width: 5.4
#| fig-height: 2.5
...
```{r}
#| label: adaptive-importance-fig3
#| echo: false
#| fig-width: 5.4
#| fig-height: 2.5
...
```{r}
#| label: adaptive-importance-fig4
#| echo: false
#| fig-width: 5.4
#| fig-height: 2.5
...
```{r}
#| label: adaptive-importance-fig5
#| echo: false
#| fig-width: 5.4
#| fig-height: 2.5
...
$$
Both components have tails heavy enough for finite variance in this example;
the broader component increases sampling away from the bulk.
...
#| echo: false
#| fig-cap: >-
#|   Target (black) and importance (blue) densities, with importance weights below.
...
x <- 10 + 3 * rt(B, df = 3)
x[mix_indicator == 0] <- 10 + 10 * rt(sum(mix_indicator == 0), df = 3)
...
Examine weights, weighted integrand values, and ESS to detect instability.
These checks cannot guarantee accuracy or reveal every missed region.
...
c(sd(tmp), sqrt(mu_hat * (1 - mu_hat))) # Est. standard deviations
```
...
We need about `r round(mu_hat * (1 - mu_hat) / var(tmp))` times as many naive
samples to achieve the same precision as importance sampling.

\medskip\pause

Benchmarking would show that the extra computing time for importance sampling
is small compared to the reduction in variance.
