...
$$
are the _standardized weights_. This works irrespectively of the value of the
normalizing constant $c$, and also if an unnormalized $g$ is used.
...
estimates. For $c \neq 1$ it is necessary to estimate $c$ as $\hat{c} =
...
\begin{align*}
  \mu            & = (2 \pi)^{-p/2} \int e^{- \frac{1}{2} \sum_{i = 2}^p \alpha^2 x_{i-1}^2 - 2\alpha x*i x*{i-1}} e^{-\frac{\lVert x \rVert_2^2}{2}} \mathrm{d} x \\
                 & = \int \underbrace{e^{- \frac{1}{2} \sum*{i = 2}^p \alpha^2 x*{i-1}^2 - 2\alpha x_i x*{i-1}}}_{h(x)} f(x) \mathrm{d} x
\end{align*}
