...
x_pos <- x[x > 0]
w_star[x > 0] <- exp(
  (x_pos - 10)^2 / 18 - x_pos + 7 * log(x_pos)
)
...
$$
and that we want to estimate $\E(f(X))$ where $f(X)$ is large near the origin.
...
$$
  g_{\mathrm{mix}}(x) = 0.8 \cdot \mathcal{N}(10, 3^2)
                        + 0.2 \cdot \mathcal{N}(10, 10^2)
$$
