...
$$
  N \approx \left(
    \frac{1.96\,\hat{\sigma}_{\mathrm{pilot}}}{\varepsilon}
  \right)^2.
$$
...
Return to the $\mathrm{Gamma}(3,1)$ target and estimating $\E_f[X] = 3$.
The Laplace proposal $g_L = \mathcal{N}(2,2)$ has a right tail that is too light:
...
defensive_colors <- c(
  target = "black", laplace = "royalblue", mixture = "darkorange"
)
defensive_lines <- c(
  target = "solid", laplace = "dashed", mixture = "dotdash"
)
defensive_labels <- c(
  target = "Target", laplace = "Laplace", mixture = "Mixture"
)

p_defensive_densities <- gamma_defensive |>
  filter(x <= 12) |>
  pivot_longer(c(target, laplace, mixture), names_to = "density") |>
  ggplot(aes(x, value, color = density, linetype = density)) +
  geom_line() +
  scale_color_manual(values = defensive_colors, labels = defensive_labels) +
  scale_linetype_manual(values = defensive_lines, labels = defensive_labels) +
  guides(
...
p_defensive_weights <- gamma_defensive |>
  filter(x > 0) |>
  transmute(x, laplace = target / laplace, mixture = target / mixture) |>
  pivot_longer(c(laplace, mixture), names_to = "proposal", values_to = "weight") |>
  ggplot(aes(x, weight, color = proposal, linetype = proposal)) +
  geom_line() +
  scale_y_log10(
    breaks = 10^c(0, 10, 20, 30),
    labels = function(breaks) parse(text = paste0("10^", log10(breaks)))
  ) +
  scale_color_manual(values = defensive_colors, labels = defensive_labels) +
  scale_linetype_manual(values = defensive_lines, labels = defensive_labels) +
  guides(
  ...
  labs(
    x = expression(x), y = expression(f(x) / g(x)),
