...
$$
  \hat{\mu}_{\mathrm{MC}} = \frac{1}{n} \sum_{i=1}^n h(X_i) \rightarrow \mu = \E h(X_1) = \int h(x) f(x) \ dx
$$
...
where
$$
  \sigma^2_{\mathrm{MC}} = \var h(X_1) = \int (h(x) - \mu)^2 f(x) \ dx.
$$
...
where
$$
  \sigma^{*2}_{\mathrm{IS}} = \var \big( h(X_1)w^*(X_1)\big) = \int (h(x) w^*(x) - \mu)^2 g(x) \ dx.
$$
...
where $$
\begin{aligned}
  \sigma^2_{\mathrm{IS}} & = \var \big(h(X_1) w^*(X_1)\big) \\
...
\begin{aligned}
  \mu_{t+1}      &= \frac{\sum_{i=1}^n w_i^* x_i}{\sum_{i=1}^n w_i^*},\\
  \sigma^2_{t+1} &= \frac{\sum_{i=1}^n w_i^* (x_i - \mu_{t+1})^2}{\sum_{i=1}^n w_i^*}.
\end{aligned}
$$
