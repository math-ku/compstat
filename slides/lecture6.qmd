...
Computing the variance of IS with normalized weights is more
...
Visualize the weight distribution with, for example, histograms or box plots to identify
degeneracies.
...
ESS measures how many samples are effectively
...
CV near 0 means weights are nearly uniform---great! As a rule of thumb, CV
below 1 ($\mathrm{ESS} > n/2$) is good.
...
The mean and spread of $q$ should roughly match the regions of large
$|h(x)|f(x)$.
...
3. Update the importance distribution parameters based on weighted samples.\pause
...
Examine weight distribution and check effective sample size to ensure reliable estimates.
