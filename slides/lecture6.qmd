...
$$
  \hat{\mu}_{\mathrm{MC}} = \frac{1}{N} \sum_{i=1}^N h(X_i) \rightarrow \mu = \E h(X_1) = \int h(x) f(x) \ \mathrm{d}x
$$
...
$$
  \frac{1}{N} \sum_{i=1}^N h(X_i) \overset{\mathrm{approx}} \sim
  \mathcal{N}(\mu, \sigma^2_{\mathrm{MC}} / N)
$$
where
$$
...
$$
  \hat{\sigma}^2_{\mathrm{MC}} = \frac{1}{N - 1} \sum_{i=1}^N (h(X_i) - \hat{\mu}_{\mathrm{MC}})^2,
$$
then the variance of $\hat{\mu}_{\mathrm{MC}}$ is estimated as
$\hat{\sigma}^2_{\mathrm{MC}} / N$ and a standard 95% confidence interval for
$\mu$ is
$$
...
More commonly, just run until the empirical confidence interval is sufficiently
narrow.
...
p1 <- tibble(x, target, importance) |>
  ...
  geom_line() +
  scale_color_manual(values = c(target = "black", importance = "steelblue")) +
...
#|   distribution using a N(10,3) importance distribution.
#| fig-width: 5
#| fig-height: 2.5
...
Again by the LLN
$$
  \hat{\mu}_{\mathrm{IS}}^* \rightarrow \E h(X_1) w^*(X_1) = \mu  \quad \text{as } N \to \infty.\pause
$$
And by the CLT
$$
  \hat{\mu}_{\mathrm{IS}}^* \overset{\mathrm{approx}} \sim
  \mathcal{N}(\mu, \sigma^{*2}_{\mathrm{IS}} / N)
$$
where
$$
  \sigma^{*2}_{\mathrm{IS}} = \V \big( h(X_1)w^*(X_1)\big) = \int (h(x) w^*(x) - \mu)^2 g(x) \ \mathrm{d}x.
$$

## Importance Sampling Variance

The IS variance can be estimated analogously with the MC variance,
$$
  \hat{\sigma}^{*2}_{\mathrm{IS}} = \frac{1}{N - 1} \sum_{i=1}^N (h(X_i)w^*(X_i) - \hat{\mu}_{\mathrm{IS}}^*)^2.
$$
...
$$
  \hat{\mu}^*_{\mathrm{IS}} \pm 1.96 \frac{\hat{\sigma}^*_{\mathrm{IS}}}{\sqrt{N}}.
$$
...
#| fig-height: 3
#| fig-cap: >-
#|   Densities, integrand, and weights for the importance sampling example with
...
p1 <- ggplot(dens_df, aes(x, value, color = Type, linetype = Type)) +
  geom_line() +
...
#| fig-cap: >-
#|   Densities, integrand, and weights for the importance sampling example with
#|   Student-$t$ as importance distribution.
...
p2 <- ggplot(dens_df_t, aes(x, value, color = Type, linetype = Type)) +
  geom_line() +
  labs(y = "Density / Integrand") +
  scale_color_manual(
    values = c(
  ...
  scale_linetype_manual(
    values = c(target = "solid", importance = "dashed", integrand = "dotdash"),
    labels = c(
      target = expression(f(x)),
      importance = expression(g(x)),
      integrand = expression(h(x) * f(x))
...
And
$$
  \mu = \frac{\int h(x) w^*(x) g(x) \ \mathrm{d}x}{\int w^*(x) g(x) \ \mathrm{d}x},
$$
where
$$
  w^*(x) = \frac{q(x)}{g(x)}.
...
$$
  \hat{\mu}_{\mathrm{IS}} = \frac{\sum_{i=1}^N h(X_i) w^*(X_i)}{\sum_{i=1}^N w^*(X_i)} \pause
...
Let's pretend that we do not know the normalizing constant of the target
($\mathrm{Gamma}(8,1)$) and importance ($\normal(10, 3^2)$) distributions. Then
$$
  w^*(x) = \frac{x^7 \exp(-x)}{\exp{\left(-\frac{(x-10)^2}{18}\right)}}
         = \exp\left(\frac{(x-10)^2}{18} - x + 7 \log(x)\right).
$$
...
$$
  \nabla t(c\mu, c)^T \left(\begin{array}{cc} \hat{\sigma}^{2}_{\mathrm{IS}} & \gamma \\ \gamma & \sigma^2_{w^*}
...
$$
  \hat{\mu}_{\mathrm{IS}} \overset{\mathrm{approx}}{\sim}
  \mathcal{N}\left(\mu, \frac{\sigma^{2}_{\mathrm{IS}} + \mu^2 \sigma_{w^*}^2 - 2 \mu \gamma}{c^2 N} \right).
$$
...
# Add tangent plane
persp3D(x_seq, y_seq, z_tangent, add = TRUE, col = "grey", alpha = 0.6)
...
#|   95% confidence bands using normalized weights.
...
Visualize the weight distribution with, for example, histograms or box plots to
...
ESS measures how many samples are effectively contributing to the estimate.
...
$$
and that we want to estimate $\E(f(X))$ where $f(X)$ is large near
the origin.
...
$$
  \mathcal{N}(\mu, I_{2})
$$
with large $|\mu|$, most samples are far from the origin, so $f(X)$ is nearly
...
CV near 0 means weights are nearly uniform---great! As a rule of thumb, CV below
1 ($\mathrm{ESS} > N/2$) is good.
...
The mean and spread of the importance distribution should roughly match the
regions of large $|h(x)|f(x)$.
...
Connection comes from the second-order Taylor expansion of $\log f(x)$ at the
mode $x^*$,
$$
...
$$
  f(x) \approx f(x^*) \exp\left(-\frac{1}{2}(x - x^*)^T \Lambda (x - x^*)\right)
$$
...
2. Compute the Hessian $\Lambda$ of the first-order Taylor approximation of
   $-\log f(x)$ around $x^*$.\pause
3. Use $\mathcal{N}(x^*, \Lambda^{-1})$ as importance distribution.
...
So the Laplace approximation is $\mathcal{N}(2, 2^2)$.
:::::
...
ggplot(df, aes(x = x, y = value, color = Density, linetype = Density)) +
  geom_line() +
  scale_color_manual(values = c(f = "black", laplace = "steelblue")) +
...
1. Choose an initial importance distribution $g_0(x)$.\pause
2. Draw samples from $g_0$ and compute weights.\pause
...
adaptive_is <- function(N = 1000, n_iter = 5, q_init_mean = 0, q_init_sd = 1) {
...
plot_step <- function(iter) {
  ...
  ggplot(df, aes(x = x, y = value, color = Density, linetype = Density)) +
    ...
    labs(y = "Density", subtitle = sprintf("ESS = %.0f, CV = %.1f", ess, cv))
}
```
...
$$
  g_{\mathrm{mix}}(x) = 0.8 \cdot \mathcal{N}(10, 3^2) + 0.2 \cdot \mathcal{N}(10, 10^2)
$$
...
#| echo: false
#| fig-cap: >-
#|   Target (black) and importance (blue) densities with samples from the
#|   importance distribution.
...
#| fig-width: 2.7
#| fig-cap: >-
#|   Target (black) and importance (orange) densities with samples from the
#|   defensive importance mixture distribution.
#| echo: false
...
p1 <- tibble(x = z, target = f_x, mixture = g_mix) |>
  ...
  geom_line() +
  guides(color = guide_legend(position = "inside")) +
  scale_color_manual(values = c(target = "black", mixture = "darkorange")) +
  scale_linetype_manual(values = c(target = "solid", mixture = "dashed")) +
  theme(legend.position.inside = c(0.8, 0.8)) +
  labs(y = "Density", color = "Density", linetype = "Density")
...
sim_net <- function(Aup, p) {
  ones <- which(Aup == 1)
  Aup[ones] <- sample(c(0, 1), length(ones), replace = TRUE, prob = c(p, 1 - p))
  Aup
}
```
...
Examine weight distribution and check effective sample size to ensure reliable
estimates.
...
Implement an importance sampler for computing the mean of the von Mises
distribution. Plot the result and check convergence towards $\mu$.
