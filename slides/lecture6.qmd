...
p1 <- tibble(x, target, importance) |>
  ...
  geom_line() +
  scale_color_manual(
    values = c(target = "steelblue", importance = "darkorange")
  ) +
  scale_linetype_manual(values = c(target = "dashed", importance = "solid")) +
...
Redesign importance distribution to better match the n
...
$\operatorname{E}[f(X)]$ where $f(X)$ is large near the origin.

\medskip\pause

If we use a importance density $g(x) = \mathcal{N}(\mu, I_{10})$ with large
$|\mu|$, most samples are far from the origin, so $f(X)$ is nearly zero and
...
It is defined as
$$
\mathsf{ESS} = \frac{(\sum_{i=1}^n w_i)^2}{\sum_{i=1}^n w_i^2}
$$
where $w_i$ are the importance weights.

\medskip\pause

If all weights are equal, $\mathsf{ESS} = n$. If one weight dominates,
$\mathsf{ESS} \approx 1$, and the estimate is unreliable.
...
Try various importance distributions
