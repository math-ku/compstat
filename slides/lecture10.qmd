...
$$
  \begin{aligned}
  Q(\theta \mid \theta')
    &= \E_{Y \mid X, \theta'} \log p (Y \mid \theta) \\ \pause
    &= \sum_{k=1}^6 \E_{Y \mid X, \theta'}(Y_k) \log \pi_k(\theta) \\ \pause
...
$$
  ...
  \nabla_{\theta'} \nabla_{\theta} H(\hat{\theta} \mid \hat{\theta}) =
  \nabla^2_{\theta} H(\hat{\theta} \mid \hat{\theta}).
$$
...
So the error at step $t+1$ is approximately a linear transformation of the error
at step $t$.
...
Relax the M-step: instead of fully maximizing $Q(\theta \mid \theta^{(t)})$, we
just require an **increase**:
$$
...
- E-step: Compute
  $Q(\theta \mid \theta') = \E_{X_\text{miss} \mid X_\text{obs}, \theta'}\left( \log p(X_\text{obs}, X_\text{miss} \mid \theta)\right)$.
