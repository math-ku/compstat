...
Suppose that $Y$ is a random variable and $X = M(Y)$. Suppose that $Y$
has density $f(\cdot \mid \theta)$ and that $X$ has marginal density
$g(x \mid \theta)$.
...
that the EM-algorithm increases the log-likelihood, $\ell(\theta)$, in each
iteration.
...
$$
\log g(x \mid \theta) - \log g(x \mid \theta') \geq Q(\theta \mid \theta') - Q(\theta' \mid \theta)
$$

\medskip\pause

In other words, the marginal likelihood **weakly** increases in each iteration.
