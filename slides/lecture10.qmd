...
moth_optim <- optim(c(0.3, 0.3), nll)
```
...
$Q$ is not typically convex, so no guarantee that the EM algorithm converges to
the global maximum.
...
The empirical Fisher information is defined as
$$
...
\log p(Y \mid X, \theta)$, we can decompose this as
$$
...
```{r q-def}
Q <- function(theta0, theta1, x = c(85, 196, 341)) {
  theta0[3] <- 1 - theta0[1] - theta0[2]
  theta1[3] <- 1 - theta1[1] - theta1[2]
  group <- c(1, 1, 1, 2, 2, 3)
  (x[group] * pi_map(theta1) / M(pi_map(theta1), group)[group]) %*%
    log(pi_map(theta0))
}
```
...
```{r gradq-def}
Q_grad <- function(theta, theta1, x = c(85, 196, 341)) {
  theta[3] <- 1 - theta[1] - theta[2]
  group <- c(1, 1, 1, 2, 2, 3)
  (x[group] *
    pi_map(theta1) /
    (M(pi_map(theta1), group)[group] * pi_map(theta))) %*%
    pi_grad(theta)
}
```
...
```{r q-hess}
iY <- -hessian(Q, theta_hat, theta1 = theta_hat)
iY
```
...
We can check against the result from method 1.
...
The log-rate of convergence can be estimated by least-squares.
...
- One gradient step on $Q$ instead of full maximization.\pause
- Useful for **complex mixture models** or latent structures.
...
switching the labels of the components, so the EM algorithm may converge to
...
Notice that the solutions have switched the labels of the mean components.
