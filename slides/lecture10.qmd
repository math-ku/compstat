...
neg_noglik_full <- mult_likelihood(
  x = c(85, 196, 341),
  group = c(1, 1, 1, 2, 2, 3),
  pi_map = pi_map,
...
moth_optim <- optim(c(0.3, 0.3), neg_noglik_full)
```
...
nll_pep <- Vectorize(function(p1, p2) neg_noglik_full(c(p1, p2)))
...
#| fig-width: 4
#| fig-height: 2.5
mutate(em_trace, diff_nll = 1e-12 + nll - min(nll)) |>
...
neg_logliks <- apply(res, 2, neg_noglik_full)
