...
A version of gradient descent that samples observations (functions) and
...
In assembly (human-readable machine) code, the code on the previous slide might
look like this:
...
C++ generates performant machine code: you no longer have to be scared of for
loops.
...
Rcpp offers many functions (sugar) that offer counterparts to R functions,
including
...
C++ is **strongly typed**: you need to specify types for return values and
arguments.
...
Premature optimization is especially useless with C++ since the compiler will
often optimize the code for you anyway.
...
We won't cover the details here, but expect much better leverage than using R's
...
1. Implement a function using Rcpp that computes the standard deviation of a
   (numeric) vector.\footnote{Don't use `Rcpp::sd()`, please!}\pause
...
We learn how to set up R packages, which is the best way to distribute R and
Rcpp code.
