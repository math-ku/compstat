...
The main reason is **performance**. C++ generates machine code,
so you no longer have to be scared of for loops or excessive
copying.
...
sum_cpp(int x) // Argument type: int
{
  return x / 3;
}
```
...
In C++, you cannot directly pass functions as argments.
...
Actually, you can, but you have to use 
...
NumericVector
f_copy(NumericVector x) {
  return pow(x, 2);
}

NumericVector
f_constref(const NumericVector& x) {
  return pow(x, 2);
}

// [[Rcpp::export]]
NumericVector
f_copyref(const bool copy) {
...
```{r vMsim3, dependson="vMsim-cpp", echo = FALSE}
#| fig-width: 5
par(mfrow = c(1, 2))
...
```{r vMsim-cpp-runtime, dependson=c("vMsim-cpp", "vM-rejection-vec", "vM-rejection"), echo = FALSE}
#| fig-width: 5
bench::mark(
...
// [[Rcpp::export]]
NumericVector
run_mean_cpp(NumericVector y, int k) {
  int n = y.size();
  int m = floor((k - 1) / 2);
  int kk = 2 * m + 1;
