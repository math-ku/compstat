...
par(
  mfrow = c(1, length(beta_mom_values)),
  mai = c(1, 0, 1, 0.0),
  oma = c(0, 4, 0.1, 0.1),
  cex = 1
)
for (beta_mom in beta_mom_values) {
  res <- gd(X, y, beta_mom = beta_mom)
  first_plot <- beta_mom == beta_mom_values[1]
  contour(
    b1,
    b2,
    z,
