...
some folder and call `testthat::test_dir()`{.r}
:::::
...
i.e. find
$$
  \theta^* = \argmin_{\theta}\Big((a - \theta_1)^2 + b(\theta_2 - \theta_1^2)^2\Big).
$$
...
```r
objective <- function(x, a = 1, b = 100) (a - x[1])^2 + b * (x[2] - x[1]^2)^2
```
...
For the Rosenbrock function, $f^* = f(a,a^2) = f(1,1) = 0$. Let's make sure this
is the case for us too!
...
```r
my_fun <- function(x) my_fun_cpp(x)
```
...
#' Function Title
#'
#' Here you describe what the function does, possibly using several lines.
#'
