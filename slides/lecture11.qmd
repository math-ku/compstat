...
Can we find conditions on the step sizes $t_k$ that ensure convergence of SGD?
**Yes!**
...
A differentiable function $f: \mathbb{R}^p \to \mathbb{R}$ is **strongly
convex** if
$$
...
$$
  f(x) = x^2
$$
\pause is strongly convex with $\mu = 2$ since $$
\begin{aligned}
...
We reach the minimum in one step with gradient descent and step size
$t = 1/L = 1/2$.

## The Robbins--Monro Convergence Theorem

Suppose $f$ is strongly convex and we have unbiased stochastic estimates of the
gradients, $g_k$, with $\E(g_k \mid x_k) = \nabla f(x_k)$ and
$$
...
$$
  t \sum_{i=1}^n \Big( \nabla f_i(x_{k + i - 1}) - \nabla f_i(x_k) \Big)
$$
which does not tend to zero for constant $t$. So the noise in the gradient
...
Fixing the exponent $a$ and picking a target rate, $t_1$, to be reached after
$k_1$ steps, we can solve for $K$ and get
$$
...
decay_scheduler <- function(t0 = 1, a = 1, K = 1, t = NULL, k1 = NULL) {
  force(a)
