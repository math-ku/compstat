...
il_df <- data.frame(
  x = xx,
  y = gausdens(xx, mu1, mu2, sigma1, sigma2, p)
)

pl <- ggplot(il_df, aes(x, y)) +
  geom_line() +
  labs(y = "Density", x = "x")
...
$$
P(Z = z \mid X = x) = \frac{\pi_z f_z(x \mid \psi_z)}{\sum_{k = 1}^K \pi_k f_k(x \mid \psi_k)}.
$$
...
$$
\hat{\theta}^{(t + 1)} = 
\left(\frac{1}{n} \sum_{i} \tau_i, \quad \frac{1}{\sum_{i=1}^n \tau_i} \sum_{i=1}^n \tau_i x_i, \quad \frac{1}{\sum_{i=1}^n (1 - \tau_i)} \sum_{i} (1 - \tau_i) x_i \right).
$$
...
We update $\mu_1$ as 
$$
\mu_1^{(t + 1)} = \frac{\sum_{i=1}^n \tau_i x_i}{\sum_{i=1}^n \tau_i},
$$
...
$\E(g_k \mid x_k) = \nabla f(x_k)$ and 
$$
\E\big(\rVert g_k(x)\lVert_2^2 \mid x_k \big) \leq A + B \lVert x_k \rVert_2^2
$$
...
$$
f(x_n) - f^* = O\left(\frac{1}{\sqrt{k}}\right). 
$$
...
$$
\E\big(f(x_n) - f^*\big) = O\left(\frac{1}{\sqrt{k}}\right).
$$
...
```{r decay-fig, echo=FALSE, dependson="decay", fig.cap="Examples of learning rate schedules. Note that the left panel has $a \\leq 1$ while the right panel has $a > 1$ (not guaranteed convergence)."}
#| fig-height: 3
#| fig-width: 5
...
$$
F(\beta) = \frac{1}{2n} \sum_{i=1}^n f_i(\beta) = \frac{1}{2n} \sum_{i=1}^n (y_i - x_i \beta)^2 + \frac{\lambda}{2} \beta^2
$$
...
f_grad <- function(beta, i, xx, yy, lambda) {
  (xx[i] * beta - yy[i]) / n * xx[i] + lambda * beta
}
...
Test your implementation against R's built-in `optimize()`. Then 
profile your code.
...
Try implementing mini-batch SGD or a learning rate schedule.
