...
convex and
$$
  \E\left(\|\nabla_\theta L(Y,\theta)\|_2^2\right) \leq A + B \| \theta \|_2^2
$$
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    % O(delta^n) with delta = 0.5
    \addplot[
      thick,
      dotted,
      samples=50,
      line label={$O(\delta^n)$}
    ] {0.5^x};
  \end{axis}
\end{tikzpicture}
...
for (i in c(1, 2, 3, 5, 10, 15, 20, steps)) {
  plot_iteration(i, x_gd, x_sgd)
}
```
...
::::: {.column width="47%"}
```{r}
#| label: step-size-illustration-06
#| echo: false
#| fig-height: 3
...
::::: {.column width="47%"}
```{r}
#| label: step-size-illustration-09
#| echo: false
#| fig-height: 3
