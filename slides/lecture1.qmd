...
Largely speaking, it is **the use of computational methods to solve statistical
...
::: {.columns}
::::: {.column width="47%"}
To get a bit of flavor about what we will be doing in the course, we will start
with a running example on a data set of angles, $\Phi$ and $\Psi$, formed
between two planes in protein structures.
:::::
...
The course can be broken down into a number of **statistical** topics, as well
as **computational** topics.
...
make sure you have *tried* each of them.
...
package](https://github.com/nielsrhansen/CSwR/tree/master/CSwR_package):
...
Generative AI (e.g. GPT, Claude, Gemini, etc.) are powerful tools.
...
- You must understand and take responsibility for the results.
...
Write your code in scripts (`.R` files), not in the console. Organize your files
in project directories, one for each project.

. . .

### Presentations

Use [quarto](https://quarto.org) or [R Markdown](https://rmarkdown.rstudio.com)
for easily interleaving code and text.
...
An R function takes a number of *arguments*, and when a function call is
evaluated it computes a *return value*.
...
First check if you can use a *verb*, which signals **intent**.
