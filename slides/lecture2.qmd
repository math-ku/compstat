...
- For a continuous random variable, the probability of \( X \) falling in a very small interval around \( x \) is very small.
- The probability density function \( f_0(x) \) tells us how likely \( X \) is to be near \( x \).
- For a small interval \((x-h, x+h)\), the probability is approximately the height of the density at \( x \) times the width of the interval.
- Mathematically:  
  \( P(X \in (x-h, x+h)) \approx f_0(x) \times (2h) \)
- This approximation gets better as \( h \) gets smaller, because the density doesn’t change much over a tiny interval.
...
These tests are ad-hoc, but they illustrate the idea of testing.
Later in the course we will learn about formal testing using the
`testthat` package.
...
plot_psi_density <- function(h, n = 1001) {
  ...
  ggplot(psi_df, aes(x = psi)) +
    ...
    geom_rug(alpha = 0.3) +
    geom_line(data = dens_df, aes(x = x, y = y), color = "blue", linewidth = 1) +
    labs(x = expression(psi), y = "density") +
    lims(y = c(0, 0.5)) +
    theme_minimal()
}
...
phipsi_plot <- function(bw) {
  phi <-  ggplot(phipsi, aes(x = phi)) +
    ...
    geom_rug(alpha = 0.3) +
    geom_density(bw = bw, color = "dark orange", linewidth = 1) +
