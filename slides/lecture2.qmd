...
numerically integrating the function. It is an example of a *functional*.
...
It allows us to write functions that act based on the class of the argument.
...
It is a simple but informal system: no formal object definition and no type
checking.
...
To use S3, we implement a \alert{generic} function via `UseMethod()`. E.g.
`plot()`:
...
Instead of having to write specialized functions, e.g. `print_lm()`,
...
Returns a list with three components: the mean, median, and standard deviation
of the input vector.
...
::: {.columns}
::::: {.column width="53%"}
The print method is written by just suffixing the class name with a dot after
the generic name.
...
\pdfpcnote{
  ...
  - Mention that people in course have different opinions
    about OOP.
}
...
assume a specific parametric form for $f_0$, e.g. the **histogram**, which we
can write as $\hat{f}$.
...
p3 <- p2 + geom_line(aes(a, d, group = x), col = "dark orange", data = res)
...
Using plug-in estimates of $\|f_0''\|_2^2$, AMISE can be used to estimate the
...
#| echo: false
#| fig-width: 5.5
#| fig-cap: "Silverman's rule of thumb bandwidth (default in `density()`, uses
#|   `bw = 'nrd'`)."
...
#| echo: false
#| fig-width: 5.5
#| fig-cap: "Sheather & Jones' bandwidth. Uses `bw = 'SJ'` (in `density()`) and
#|   `bw = 'sj'` (in ggplot2)."
...
Sample a data set from a Gaussian mixture model, varying the standard deviation
of the components. How does the different bandwidth selection methods in
`density()` perform?
