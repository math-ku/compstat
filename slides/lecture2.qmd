...
$$
  \P(X \in (x-h, x+h)) = \int_{x-h}^{x+h} f_0(z) \ \mathrm{d}z \pause \approx f_0(x) 2h.
$$
...
$$
  \begin{aligned}
    f_0(x) & \approx \frac{1}{2h} \P(X \in (x-h, x+h)) \\ \pause
           & \approx \frac{1}{2hN} \sum_{j=1}^N 1_{(x-h, x+h)}(x_j) \\ \pause
           & = \frac{1}{2hN} \sum_{j=1}^N 1_{(-h, h)}(x - x_j) \\ \pause
           & = \hat{f}_h(x).
  \end{aligned}
$$
...
$$
where
$\mathrm{MSE}_h(x) = \V(\hat{f}_h(x)) + \mathrm{bias}(\hat{f}_h(x))^2$.
