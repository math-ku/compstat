...
A paradigm based on the concept of "objects", which contain data and methods.
...
We could use a \alert{parametric} approach, where we assume that $f_0$ belongs
to a family of densities $(f_\theta)_\theta$ indexed by a parameter $\theta$.
...
assume a specific parametric form for $f_0$. One example is the **histogram**
estimator, which we can write as $\hat{f}$.
...
$$
  \P(X \in (x - h, x + h)) = \int_{x-h}^{x+h} f_0(z) \mathrm{d}z \pause
                           \approx f_0(x) 2h.
$$
...
If $K$ is a square-integrable probability density with mean zero,
$$
  \mathrm{MISE}(h) = \mathrm{AMISE}(h) + o((Nh)^{-1} + h^4)
$$
\pause where the *asymptotic mean integrated squared error* is
$$
  \mathrm{AMISE}(h) = \frac{\|K\|_2^2}{Nh}
                      + \frac{h^4 \sigma^4_K \|f_0''\|_2^2}{4}
$$
