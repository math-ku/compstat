...
$$
is a **cubic spline** with **knots** in the data points $x_i$.
...
$\symbf{S}_{\lambda}$ and penalty matrix $\symbf{\Omega}$.
...
- Boundary adjustment is important for all local smoothing methods (running
  mean, kernel smoothing, LOESS, splines).
- At the edges, standard symmetric windows/neighborhoods cannot be applied.
- Solutions include asymmetric windows, padding, extrapolation, or excluding
  boundary points.

## Example: Padding and Asymmetric Windows

```r
# Pad data at boundaries
y_padded <- c(rep(y[1], m), y, rep(y[n], m))
s_padded <- run_mean(y_padded, k)

# Asymmetric window at left boundary
s_left <- mean(y[1:k])

# Asymmetric window at right boundary
s_right <- mean(y[(n - k + 1):n])
```

- Padding reduces bias at boundaries.
- Asymmetric windows use available points when full window is not possible.
...
Use the [Matrix
package](https://CRAN.R-project.org/package=Matrix) to
convert $\symbf{S}$ to a sparse format using `Matrix::Matrix(S, sparse = TRUE)`
and then benchmark the sparse multiplication against the dense one. Parameterize
the benchmark with $n$ and $k$.
