...
$$
where $\mathbf{y} = (y_1, \ldots, y_n)^T$ and $\mathbf{S}$ is the **smoother
matrix**.
...
$$
where $\mathbf{1}_{\{j \in N_i\}}$ is 1 if $j$ is a neighbor of $i$, 0
otherwise.
...
$$
  N_i = \{i - \frac{k - 1}{2}, i - \frac{k - 1}{2} + 1, \ldots, i - 1 , i, i + 1, \ldots, i + \frac{k - 1}{2}\}
$$
(for $k$ odd.)
...
A more efficient implementation can be defined by, assuming $y$ is symmetric, we
use the identity
$$
...
parallel_square <- function(x) {
  result <- foreach(i = seq_along(x), .combine = c) %dopar%
    {
      x[i]^2
    }
  result
...
it should prints "Future is done!" and return the value 42. Try accessing the
value of the future before and after the computation is complete.
