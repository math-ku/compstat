...
$(f(x_i))$ of the form $\symbf{S} \symbf{y}$ for a _smoother matrix_
$\symbf{S}$ is called a _linear smoother_.
...
With $\text{df} = \text{trace}(\symbf{S}) = \sum_{i=1}^n S_{ii}$ we
replace $S_{ii}$ in LOOCV by $\text{df} / n$ to get the **generalized**
