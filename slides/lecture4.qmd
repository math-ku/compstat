...
$$
  \hat{\bm{f}} = (\hat{f}_i) = \mathbf{S} \mathbf{y}.
$$

. . .

$\hat{\bm{f}}$ is an estimator of the vector $(f(x_i))$.

\pdfpcnote{
  The vector f̂ can be computed as a linear function of y.
  S is a relatively sparse n × n matrix.
}
...
$$
with $\boldsymbol{\Phi}_{ij} = \varphi_j(x_i)$, and
$$
\begin{aligned} 
...
\pdfpcnote{
  Can rewrite penalty term in quadratic form $\lambda \beta^T \mathbf{\Omega}
  \beta$.
}
...
See [slide source](/slides/lecture4.qmd) for implementation
of `pen_mat()`.

## Fitting a Smoothing Spline

We implement the matrix-algebra directly for computing $\mathbf{S}_{\lambda}
\mathbf{y}$.
...
With $\mathrm{df} = \mathrm{trace}(\mathbf{S}) = \sum_{i=1}^n S_{ii}$ we
replace $S_{ii}$ in LOOCV by $\mathrm{df} / n$ to get the **generalized**
...
$$\hat{\beta}_i(\lambda) =
\frac{\hat{\beta}_i}{1 + \lambda \gamma_i}.$$
--

\pause\medskip

The smoothed values, $\widetilde{\mathbf{U}} \hat{\beta}(\lambda)$, are computed
...
convert $\mathbf{S}$ to a sparse format using `Matrix::Matrix(S, sparse = TRUE)`
