...
digraph Futures {
  ...
  FutureCreate [label="Future created", fillcolor="#bbdefb"];
  MainWork [label="Main thread does other work", fillcolor="#c8e6c9"];
  FutureRun [label="Future runs in parallel", fillcolor="#ffe0b2"];
  Wait [label="Main thread waits if not ready", fillcolor="#ffcdd2"];
  Receive [label="Main thread receives result", fillcolor="#d1c4e9"];

  FutureCreate -> FutureRun;
  FutureCreate -> MainWork;
  MainWork -> Wait;
  FutureRun -> Receive;
  Wait -> Receive;
}
