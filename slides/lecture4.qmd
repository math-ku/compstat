...
Available on Linux and macOS, but not Windows or RStudio. *Forks* the current R
process, instead of starting a new one. But *not* recommended because of
...
Primarily used when you have various independent tasks that you want to run in
parallel.
:::::
:::
...
$$
where $\symbf{1}_{\{j \in \mathcal{N}_i\}}$ is 1 if $j$ is a neighbor of $i$, 0 otherwise.
...
nn_bench <- bench::press(N = c(512, 1024, 2048, 4196), {
  k <- 11
  y <- rnorm(N)
...
Replace $S_{ii}$ in LOOCV by $\mathrm{df} / N$, with
$\mathrm{df} = \operatorname{trace}(\symbf{S}) = \sum_{i=1}^N S_{ii}$, to get the
...
$\widetilde{\symbf{U}}$) to get coefficients
$\hat{\symbf{\beta}} = \widetilde{\symbf{U}}^Ty$.
