...
ggplot(
  bind_rows(df1, df2),
  aes(x, y, fill = method, color = method)
) +
  geom_area(
    alpha = 0.4,
    position = "identity",
  ...
  geom_line() +
  scale_fill_manual(
    values = c(standard = "transparent", binning = "blue")
  ) +
  scale_color_manual(
    values = c(standard = "black", binning = "transparent")
  ) +
...
#| fig-width: 5.3
#| echo: false
#| fig-cap: Errors due to binning decrease as the length of the data sequence
#|   increases.
...
df <- data.frame(
  ...
  diff = c(
    ...
    kern_dens(x[1:1024], 0.2)$y -
      kern_dens_bin(x[1:1024], 0.2)$y,
