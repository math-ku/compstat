...
profiler_colors <- c(f = "royalblue", g = "darkorange", h = "grey35")
profiler_code <- c("function(x) {", "  f(x)", "  g(x)", "  h(x)", "}")
...
draw_profiler_sample <- function(step) {
  ...
  text(0.01, 0.27, "sample tally", adj = 0, family = "sans", cex = 0.8)

  tally_y <- c(f = 0.19, g = 0.12, h = 0.05)
  observed <- profiler_samples[seq_len(step)]

  for (fun in names(tally_y)) {
    text(0.06, tally_y[fun], paste0(fun, "()"), family = "mono", cex = 0.85)
...
profiler_figure_base <- file.path(profiler_figure_path, "sampling-profiler")
dir.create(profiler_figure_path, recursive = TRUE, showWarnings = FALSE)
...
tools to visualize profiling output.^[It can be activated directly in RStudio
(but not Positron).]
...
The result is an interactive webpage that opens in a tab in RStudio.
...
`gamma()` receives the most samples, and its cost appears directly on its source
line.
...
Almost all samples land on the loop body---not the `for` statements. Numerical
work and R's loop overhead are bundled together.
...
loop_bench <- bench::mark(sapply = sapply(x, function(x_i) 10 * x_i), loop = {
  y <- numeric(length(x))
...
mutate(res2, expr = as.character(expression), median = as.numeric(median)) |>
...
```{r}
#| label: plot-kern-dens-benchmark
#| fig-width: 4
#| echo: false
#| fig-cap: The standard density estimator as a black line and the binning
#|   estimate as a filled area.
...
df1 <- tibble(x = y1$x, y = y1$y, method = "standard")

df2 <- tibble(x = y2$x, y = y2$y, method = "binning")

ggplot(bind_rows(df1, df2), aes(x, y, fill = method, color = method)) +
  geom_area(alpha = 0.4, position = "identity", color = "transparent") +
  geom_line() +
  scale_fill_manual(values = c(standard = "transparent", binning = "blue")) +
  scale_color_manual(values = c(standard = "black", binning = "transparent")) +
...
```{r}
#| label: test-kern-dens-bench
#| fig-width: 5.3
#| echo: false
#| fig-cap: Absolute errors due to binning decrease as the length of the data
#|   sequence increases.
