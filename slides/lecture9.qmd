...
Find a feasible point $\theta_0$ by solving the optimization problem
...
If $s^* \leq 0$, then $\theta_0$ is feasible.
...
$$
  Q(\theta \mid \theta_n) = \E_{\theta_n}\left( \log p(X, Z \mid \theta) \mid X \right).
$$
...
$$
  \ell(\theta) = \log p(X \mid \theta)
               \geq \E_{q(Z)}[\log p(X,Z \mid \theta)] - \E_{q(Z)}[\log q(Z)].
$$
...
- Each experiment: pick a coin at random (latent variable $Z_i$) and flip it 10
  times.\pause
...
would be
$$
  p(X,Z \mid \theta) = \sum_i \left( \symbf{1}_{Z_i = A} \textrm{Bin}(X_i \mid 10, \theta_A) + \symbf{1}_{Z_i = B} \textrm{Bin}(X_i \mid 10, \theta_B) \right).
$$
...
$$
  \log p(X_i, Z_i \mid \theta) = \symbf{1}_{Z_i = A} \log \textrm{Bin}(X_i \mid 10, \theta_A)
...
$$
  Q(\theta \mid \theta_n) = \sum_i \gamma_{i,n} \log \textrm{Bin}(X_i \mid 10, \theta_A)
...
$$
  \theta_{A,n + 1} = \frac{\sum_i \gamma_{i,n} X_i}{\sum_i 10 \gamma_{i,n}}, \quad \theta_{B,n + 1}
...
legend(
  "bottomright",
  legend = c(expression(theta[A * "," * n]), expression(theta[B * "," * n])),
  col = c("blue", "red"),
  lty = c(1, 1),
  pch = c(19, 19)
)
