...
\begin{algorithm}[H]
  ...
  \Repeat{$m/t < \varepsilon$ }{
    $\theta \gets \operatorname*{argmin}_{\vartheta} tH(\vartheta) + \phi(\vartheta)$\;
    $t \gets \mu t$\;
  }
  \Return{$\theta$}
\end{algorithm}
...
$$
  Y_{A_j} \mid X
          = x
          \sim \textrm{Mult}\left( \frac{p_{A_j}}{M(p)_j}, x_j \right).
$$
...
would be
$$
  p(X,Z \mid \theta) = \sum_i \left( \symbf{1}_{Z_i = A} \text{Binomial}(X_i \mid 10, \theta_A) + \symbf{1}_{Z_i = B} \text{Binomial}(X_i \mid 10, \theta_B) \right).
$$
...
$$
  \log p(X_i, Z_i \mid \theta) = \symbf{1}_{Z_i = A} \log \text{Bin}(X_i \mid 10, \theta_A)
...
$$
  Q(\theta \mid \theta_n) = \sum_i \gamma_{i,n} \log \text{Bin}(X_i \mid 10, \theta_A)
                            + \sum_i (1 - \gamma_{i,n}) \log \text{Bin}(X_i \mid 10, \theta_B).
$$
...
$$
  \begin{aligned}
    \gamma_{i,n} & = \E_{\theta_n}\left(\symbf{1}_{Z_i = A} \mid X_i\right)                                                                                      \\
                 ...
                 & = \frac{\text{Binomial}(X_i \mid 10, \theta_{A,n})}{\text{Binomial}(X_i \mid 10, \theta_{A,n}) + \text{Binomial}(X_i \mid 10, \theta_{B,n})}.
  \end{aligned}
$$
