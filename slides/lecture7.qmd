...
$$
  \operatorname*{minimize}_{\theta}\big( - \log f(\theta \mid x)\big),
$$
...
ggplot(df, aes(x, y)) +
  ...
  geom_point(data = pts, aes(x, y), label = expression(x[2]), nudge_y = 0.3) +
  scale_linetype_manual(
...
gd_taylor_illustration <- function(f, f_prime, x0, t) {
  ...
  ggplot(df_long, aes(x)) +
    ...
    scale_color_manual(
      ...
      labels = c(y = expression(f(x)), taylor = expression(hat(f)(x)))
    ) +
    scale_linetype_manual(
      values = c(y = "solid", taylor = "dashed"),
      labels = c(y = expression(f(x)), taylor = expression(hat(f)(x)))
    ) +
    labs(y = expression(f(x)), x = "x", color = NULL, linetype = NULL) +
    theme_void()
}
...
It measures how fast the gradient of $f$ can change, relative to the change in
$x$.
...
- Small relative change in $x$:
  $$
  \frac{\lVert x^{(k+1)} - x^{(k)} \rVert_2}{\lVert x^{(k)} + 10^{-q}\rVert_2}
  \leq \epsilon
  $$
  \pause
- Small relative change in $f$:
  $$
  \frac{f(x^{(k+1)}) - f(x^{(k)})}{f(x^{k})}
  \leq \epsilon
  $$
:::::
...
f <- function(x) log(1 + exp(x)) + 0.5 * (x - 2)^2

curve(f, from = -2, to = 5)

f_prime <- function(x) exp(x) / (1 + exp(x)) + (x - 2)
```
