...
to solve for $\beta$, which uses matrix factorization (e.g., Cholesky
decomposition) to solve the linear system.
...
For OLS, the model is exact: one full Newton step reaches the minimum.
:::::
...
For general objectives, a full Newton step can increase $H$. **Damped Newton**
uses
$$
  \theta_{n+1} = \theta_n - \gamma \nabla^2 H(\theta_n)^{-1} \nabla H(\theta_n),
$$
...
- When $\gamma = 1$, we enter the **pure** phase and $\gamma$ will remain
  $1$.\pause
...
#| fig-height: 2.25
#| out-width: "\\linewidth"
#| fig-cap: "Newton converges a few iterations."
...
\begin{overlayarea}{\textwidth}{1.2cm}
  ...
  \only<3->{
    ...
    \par\medskip
    {\footnotesize Iteration counts were found in advance; calibration is excluded.}
  }
\end{overlayarea}
