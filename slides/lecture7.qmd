...
\pdfpcnote{
  These are actual optimization runs on simulated binary responses. All three
  examples use the same ridge logistic objective and penalty. The stated
  condition numbers describe the design Gram matrix, not the changing Hessian.
  The signal strength is held fixed across the simulated problems.

  In this first example, the design Gram matrix has condition number 5. The
  two panels use identical objective values from the same optimization runs;
  only the horizontal coordinate changes. Newton wins by iteration count,
  while GD wins by total estimated work. Newton's five iterations cost about
  408 GD iterations, compared with GD's 218 iterations.

  A separate high-accuracy Newton fit supplies the reference minimum. Its
  remaining objective gap is bounded by the squared gradient norm divided by
  twice the ridge penalty, and is below 1e-16. Computing this reference and
  generating the data are excluded from both methods' work. Each curve ends
  at the first iterate reaching the target. Values below the target are
  displayed at the target. The vertical axis is logarithmic. The horizontal
  axes use log(1 + x), retaining the common starting point at zero iterations
  and zero work. Points mark every Newton step and the final GD step.

  GD receives the known global Lipschitz bound L = ||X|| squared / (4n) + lambda
  from the design construction. It does not form a Hessian to choose a step
  size. Newton rebuilds its Hessian at every iteration and uses a Cholesky
  factorization and Armijo backtracking with constant 1/4 and halving.

  Work is a leading-order dense arithmetic estimate, not measured time.
  Accepted fitted probabilities are reused, so one GD iteration costs about
  4np operations: one matrix-vector product for the gradient and one for the
  next objective evaluation. Newton additionally weights X, forms a symmetric
  Gram matrix, factors it, and solves two triangular systems. Every rejected
  line-search evaluation is charged another 2np operations. Scalar operations,
  data generation, and the common initial evaluation are excluded. Neither
  optimizer exploits the special structure used to generate the design.
}
...
A large Hessian makes Newton costly.
:::::
...
Curvature information pays off.
:::::
:::

\pdfpcnote{
  Each panel compares GD and Newton on the same logistic regression problem.
  The problems differ between panels. Work is measured in GD-iteration
  equivalents separately for each problem, so compare the methods within a
  panel rather than comparing absolute computational costs across panels.

  In the larger problem, GD uses 132 work units and Newton about 541. In the
  smaller, ill-conditioned problem, GD uses 791 and Newton about 27.
}
