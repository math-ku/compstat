...
# And a ggplot2 contoour plot
base_plot <- ggplot(beta_df, aes(beta1, beta2)) +
  contour_scale +
  geom_point(
    data = optimum_df,
    color = "darkorange",
...
step_size_illustration <- function(t = NULL, maxit = 35, line_search = FALSE) {
  res <- gd(x, y, t = t, maxit = maxit, line_search = line_search)
  df <- t(res$betas)
...
step_size_illustration(0.0115, maxit = 35)
```
...
#| fig-width: 5
#| fig-height: 3.25
#| fig-cap: The step size is well-adjusted!
...
Set $\gamma \in (0, 1)$ and $\alpha \in (0, 1/2]$. At each iteration, start with
...
backracking_illustration <- function() {
  ...
  labels <- c(
    ...
    f2 = expression(f(x) - t * nabla * group("||", f(x), "||")[2]^2),
    f3 = expression(f(x) - t * alpha * nabla * group("||", f(x), "||")[2]^2)
  )
  ...
  ggplot(df_long, aes(t, y)) +
    ...
    scale_color_manual(
      values = c(f1 = "black", f2 = "grey50", f3 = "steelblue"),
      labels = labels
    ) +
    scale_linetype_manual(
      values = c(f1 = "solid", f2 = "dotted", f3 = "dotted"),
      labels = labels
    ) +
    labs(color = NULL, linetype = NULL)
}
...
step_size_illustration(line_search = TRUE)
```
...
search.](../images/gd-stepsize-linesearch.png)
{width=50%}
...
conditioning.](../images/gd-conditioning.png)
{width=70%}
...
![The Newton method, with approximation.](../images/newton-approx.png)
{width=52%}
