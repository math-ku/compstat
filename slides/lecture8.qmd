...
Typically, we have output from some function and want to check that it meets our
expectations.

## Choosing Tests

Where should the expected result come from?

\medskip\pause

- **Known answers:** An optimizer should find the minimum of $f(x) = (x - 3)^2$
  at $x = 3$. \pause
- **Trusted implementations:** Compare Poisson regression estimates with `glm()`
  on the same data. \pause
- **Mathematical properties:** Compare an analytical gradient with finite
  differences. \pause
- **Boundary cases and invalid inputs:** Start at the minimum, try one
  parameter, or supply incompatible dimensions.
- **Regression tests:** Check that a bug that was fixed does not reappear.

\medskip\pause

Derive expected results independently of the code being tested.
...
It provides a simple syntax for writing tests and integrates well with
RStudio/Positron.
...
::: {.columns}
::::: {.column width="46%"}
**testthat** works inside R packages.

\medskip\pause

Tests are organized in files inside the `tests/testthat/` directory of an R
package.
:::::
...
We will learn about package development later in the course.
...
## `expect_equal()` with Tolerance

Use `expect_equal()` with a specified tolerance to handle this.

```{r}
#| label: numerical-equality-tolerance
...
Okay in the simplest of cases and easy to get going with, requires cleanup and
iterative sourcing and re-running of code.
...
::::: {.column width="47%"}
### RStudio/Positron Breakpoints

Visual breakpoints can be set in RStudio or Positron by clicking in the margin
next to a line number.
...
\pdfpcnote{
  - Show this interatively!\\
}

## AI Debugging

Coding agents (Codex, Claude, Gemini, etc.) are getting better at debugging
code.

\medskip\pause

Model Context Protocol (MCP) tools can now be used to let agents interactively
debug code.

\medskip\pause

Your knowledge of the problem domain still matters. Explain the intended
behavior, provide a reproducible example, and check that the proposed fix makes
sense.
...
$$
where $\symbf{X}$ is an $N \times p$ matrix of predictors and $\symbf{y}$ is our
response.
...
$$
  \hat{\symbf{\beta}}
  = \argmin_{\symbf{\beta}} \left( \frac{1}{N} \sum_{i=1}^N \exp(\symbf{x}_i^T \symbf{\beta}) - y_i \symbf{x}_i^T \symbf{\beta} \right).
$$
...
1. Download and open [`debugging.R`](R/debugging.R) in RStudio or Positron.
   \pause
...
6. Run `newton_method()` again. Execution should stop at the first breakpoint.
   Use the RStudio/Positron interface to step through the code and inspect
   variables. What do you see?\pause
...
We introduce the expectation-maximization (EM) algorithm, which is a general
method for maximum likelihood estimation with latent variables.
