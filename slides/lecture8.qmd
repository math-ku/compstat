...
\begin{center}
  \begin{tikzpicture}[node distance=2cm,>=stealth,rounded corners]
    ...
    \node (B) [draw, rectangle, right of=A, node distance=3.5cm] {Write tests};
    \node (C) [draw, rectangle, right of=B, node distance=3.5cm] {Run tests};
    \node (D) [draw, diamond, aspect=2, below of=C, node distance=2.5cm] {Tests pass?};
    \node (F) [draw, rectangle, left of=D, node distance=3.5cm] {Fix code};
...
We can measure test coverage using the **covr** package, which tracks which
lines of code are executed when we run our tests.

\medskip

This is somewhat out of scope for this course, but you can read more about it in
the [covr documentation](https://covr.r-lib.org/).
...
When stuck, a good strategy is to explain the code line-by-line to someone else.
...
If you don't have anyone around, why not try explaining it to a rubber duck?!
...
We won't cover it here (or even later), but it **is** possible: see
...
res_glm <- glm(y ~ x, family = "poisson")
```
...
4. Create a test for the `newton_method()` function using the `testthat` package
   that fails.\pause
4. Start debugging by setting some breakpoints in `newton_method()`.\pause
5. Run `newton_method()` again. Execution should stop at the first breakpoint.
   Use the RStudio interface to step through the code and inspect variables.
   What do you see?\pause
6. There are two bugs in `newton_method()`. Can you find and fix them?

## Next Time

### Likelihood Optimization

We continue exploring different aspects of optimization.

\medskip\pause

We focus on likelihood-based methods.

\medskip\pause

And also introduce algorithms for solving constrained optimization problems.
